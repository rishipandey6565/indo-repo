...
def process_json_file(json_path: Path):
    ...
    # Extract unique URLs from both today and tomorrow
    for day in ["today", "tomorrow"]:
        programs = schedule_data.get(day, {}).get("programs", [])
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for url, filename in unique_urls.items():
            # Output path changed: removed day_type
            save_path = Path(DOWNLOAD_ROOT) / channel_name / filename
            futures.append(executor.submit(download_svg, url, save_path))

        for future in as_completed(futures):
            future.result()

    # Update JSON URLs for both today and tomorrow
    for day in ["today", "tomorrow"]:
        programs = schedule_data.get(day, {}).get("programs", [])
        for program in programs:
            url = program.get("show_logo")
            if url and url in unique_urls:
                ...
                # Updated JSON string replacement without day_type
                new_path = f"{BASE_UPLOAD_URL}/{DOWNLOAD_ROOT}/{channel_name}/{filename}"
...
def main():
    ...
    if not dir_path.exists():
        print(f"Directory {SCHEDULE_DIR} does not exist.")
        return
